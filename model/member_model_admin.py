...
class phpmps_member_info(BaseModel):
    ...
    img_touxiang = peewee.CharField(max_length=255, null=True)
    is_zhizhao = peewee.BooleanField(null=True)
    img_credit = peewee.CharField(max_length=255, default='')
    img_zhizhao = peewee.CharField(max_length=255, null=True)
...
class MAdminUser(BaseModel):
    def set_vip(self, uname, cats):
        entry = phpmps_member_info.update(
            is_vip=True,
            vip_cat=','.join(cats)
        ).where(phpmps_member_info.user_name == uname)
        entry.execute()
    def cancel_vip(self, uname):
        entry = phpmps_member_info.update(
            is_vip=False,
        ).where(phpmps_member_info.user_name == uname)
        entry.execute()
    def get_all(self):
        return phpmps_member_info.select()

    def get_uncredit(self):
        return phpmps_member_info.select().where(
            (phpmps_member_info.is_credit != 1) and (phpmps_member_info.img_credit != ''))
    ...
    def get_by_username(self, uname):
        if uname == '':
            return False
        try:
            return (phpmps_member_info.get(user_name=uname))
        except:
            return False
