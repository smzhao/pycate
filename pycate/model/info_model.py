...
class MInfo(object):
    def __init__(self, cityid):
        ...
        self.db = con[config.mongo['dbname']]
        self.db.authenticate(config.mongo['dbuser'], config.mongo['dbpass'])
    ...
    def delete_by_uid(self, def_uid):
        self.tab_info.remove({'def_uid': def_uid})
    ...
    def get_by_condition(self, input, limit_num = 20):
        return self.tab_info.find(input).limit(limit_num).sort([('def_refresh_out_time', pymongo.DESCENDING)])
