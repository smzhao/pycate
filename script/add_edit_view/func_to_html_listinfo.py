...
def gen_radio_view(sig):
    ...
    for key in dic_tmp.keys():
        tmp_str = '''{{% if post_info['{0}'][0] == "{1}" %}} {2} {{% end %}}'''.format(sig['en'], key, dic_tmp[key])
...
def gen_checkbox_view(sig):
    ...
    dic_tmp = sig['dic']
    for key in dic_tmp.keys():
        tmp_str = '''{{% if "{0}" in post_info["{1}"] %}} {2}  {{% end %}}'''.format(key, sig['en'], dic_tmp[key])
        view_zuoxiang += tmp_str
...
def gen_select_view(sig):
    view_jushi = '''<span class="iga_pd_val">    '''
